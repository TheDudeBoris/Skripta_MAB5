...
%\item Prostor s úplnou mírou $\{\E^r,\lambda(X),\Mla\}$
%\item $G$ bude vždy znaèit oblast a $J$ bude znamenat kompakt
%\item funkce: $f(\x):\R\mapsto\C$
%\item Pøipomenout $\CC=\CC^0(M),$ $\CC^n(M),$ $\CC_0(M)$ a $\CC^n_0(M)$
%\item $\LLL(G),$ $\LLL^\star(G),$ $\LOC(G)$
%\item pøipomenout, že: $$f(x)\in\LLL(E,\mu) \quad \Leftrightarrow \quad|f(x)|\in\LLL(E,\mu)~\wedge f(x)\in \LA_\mu(E).$$
%\item Vìta o ekvivalentní definici tøídy $\LOC(G)$
%\item snaha o prehilbertovský prostor $\CC(\langle a,b\rangle)$
%\item snaha o prehilbertovský prostor $\LLL(G)$ a protipøíklad $\frac{1}{\sqrt{x}}\in\LLL(0,1),$ ale $\frac{1}{\sqrt{x}}\cdot\frac{1}{\sqrt{x}}=\frac{1}{x}\in\LLL(0,1)$
\item je ale $\CC(\langle a,b\rangle)$ úplný? (není) - \textcolor{red}{zmínit, okomentovat a vložit asi jako poznámku za poznámku 2.2.12, možná na vhodném místì zmínit definici úplnosti (možná už to nìkde je, teï si nejsem jistej)}
%\item $\LLL_p(G)$
%\item snaha o prehilbertovský prostor $\LLL_1(G)$ a protipøíklad $\frac{1}{\sqrt{x}}\in\LLL_1(0,1),$ ale $\frac{1}{\sqrt{x}}\cdot\frac{1}{\sqrt{x}}=\frac{1}{x}\in\LLL_1(0,1)$
%\item Vìta $f,g\in\LLL_2(G) \quad \Rightarrow \quad fg^\star\in\LLL_1(G)$
%\item ale ani $\LLL_2(G)$ není prehilbertovský
%\item Co jsou faktorové funkce
%\item znaèení $\LL_2(G)$  a $\LL_2^{(w)}(G):$ váha musí být spojitá a kladná na $G$
\end{enumerate}%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
funkcí} na $M$. Je-li \v c\'islu $n\in\N$ t\'imto zp\r usobem
...
\subsection{V\v eta -- \emph{Bolzanova-Cauchyova podm\'inka}} \label{Cauchyovo kritérium
pro stejnomìrnou konvergenci posloupnosti funkcí}

Posloupnost funkcí (\ref{1.1}) je stejnom\v ern\v e
